...
import com.short_term_course.security.CustomOAuth2UserService;
...
public class SecurityConfig {
    ...
    private final String[] GET_PUBLIC_ROUTES = {"/auth/**", "/", "/login", "/oauth2/**", "/api/categories/**", "/api/courses/**", "/api/classrooms/**"};
    ...
    //Là một cơ chế bảo mật trình duyệt nhằm cho phép hoặc từ chối các yêu cầu HTTP được gửi từ một domain khác với domain của server
    @Bean
    public CorsConfigurationSource corsConfigurationSource() {
        CorsConfiguration configuration = new CorsConfiguration();

        // Cho phép tất cả các nguồn (có thể muốn hạn chế các nguồn được phép truy cập bằng cách chỉ định danh sách các URL cụ thể thay vì "*" để tăng tính bảo mật.)
